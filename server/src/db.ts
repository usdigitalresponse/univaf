import { Availability, Position, ProviderLocation, LocationAvailability } from "./interfaces";
import { Pool } from 'pg';
...
const providerLocationAllFields = providerLocationFields.concat(providerLocationPrivateFields);
...
export async function updateLocation (data: any): Promise<ProviderLocation> {
  ...
  const setExpression = sqlFields.map(([key, _], index) => `${key} = $${index + 2}`);
...
export async function updateAvailability(
  id: string,
  { source, available, updated_at, checked_at, meta = null, is_public = true }: {
    source: string,
...
export async function listAvailability ({ includePrivate = false } = {}): Promise<LocationAvailability[]> {
  let fields = [
    "provider_location_id",
    "source",
    "available",
