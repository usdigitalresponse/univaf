...
module.exports = {
   ...
   */
  groupByExternalId(locations, systems = null, unpadIds = false) {
    ...
    for (const location of locations) {
      ...
      // If a location didn't have any of the ID systems used for grouping, it
      // still needs to get included in the output.
      if (!inAGroup) {
        byExternalId.set(`__UNIQUE__:${counter}`, new Set([location]));
        counter++;
      }
    }
  ...
  mergeGroupsWithCommonLocations(groups) {
    return (
      [...groups.values()]
        ...
        // We'll have left behind some empty groups, so drop those.
   ...
   */
  groupDuplicateLocations(locations, systems = null, unpadIds = false) {
    const byExternalId = module.exports.groupByExternalId(
      locations,
      systems,
      unpadIds
    );
    return module.exports.mergeGroupsWithCommonLocations(byExternalId);
  },
};
